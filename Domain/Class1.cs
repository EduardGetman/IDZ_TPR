...
namespace Domain
{
    class Class1
    {
        string[] name; // Названия компетенций
        int[][] positionsLevels;// Внутр чатьс табл 2
        int[] importanceCoefficient;// Коэф важности производственной функции
        int[][] employsLevels;// Внутр чатьс табл 1
        ...
        void main()
        {
            ...
            {
                ModelBuilder modelBuilder = new ModelBuilder(scale);
            ...
            {
                ModelBuilder positonBuilder = new PositionBuilder(scale);
